...
const handleRetiro = async (monto) => {
  try {
    await api.post('/atm/withdraw', { monto })
    successMsg.value = `Retiro de ${cuentaStore.moneda} ${monto.toLocaleString('es-MX')} procesado con éxito.`
    await cuentaStore.fetchBalance()
    showModal.value = true
  } catch (err) { throw err }
}

const otroRetiro = () => { showModal.value = false; successMsg.value = '' }
  ...
  <div class="page">
    ...
    <div class="atm-container">
      <div class="atm-screen">
        <h2 class="screen-title">Retiro de Efectivo</h2>
        <FormularioTransaccion tipo="retiro" :action="handleRetiro" />
      </div>
    </div>
    ...
    <b-modal
      ...
      ok-title="Volver al Dashboard" cancel-title="Hacer otro retiro"
      ok-variant="primary" cancel-variant="outline-secondary"
      @ok="volverDashboard" @cancel="otroRetiro"
    >
